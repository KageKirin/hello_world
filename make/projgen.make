...
## main target
REACTOR=bin/$(TARGET_OS)/reactor$(EXE)
...
$(REACTOR).$(PROJECT_TYPE): build/$(COMPILER).$(PROJECT_TYPE).$(TARGET_OS)
	$(SILENT) $(MAKE) -C build/$(COMPILER).$(PROJECT_TYPE).$(TARGET_OS) GENIE_OPTIONS=$(GENIE_OPTIONS) config=$(CONFIG)

all: clean projgen $(REACTOR).$(PROJECT_TYPE)
...
build-$(TARGET_OS): loadpackages embed projgen-$(TARGET_OS) $(REACTOR).$(PROJECT_TYPE)
